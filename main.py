...
from AI.greedy import get_next_direction
import os
...
def draw_score(score, best, is_new):
    ...
    text1 = font.render(f"Рахунок: {score}", True, WHITE)
...
def game_over_screen(score):
    screen.fill(BLACK)
    text1 = big_font.render("Гру закінчено", True, RED)
    text2 = font.render(f"Ваш рахунок: {score}", True, WHITE)
    text3 = font.render("Натисніть будь-яку клавішу для перезапуску", True, WHITE)
...
while True:
    ...
    if control_mode == "ШІ (Greedy)":
        mode_key = "ШІ (Greedy)"
    else:
        mode_key = "Гравець"

    FPS = {"Легко": 8, "Нормально": 12, "Важко": 18}[difficulty]
    ...
    while running:
        ...
        if control_mode == "Гравець":
            ...
            if keys[pygame.K_w] and direction != (0, 1):
                ...
                direction = (1, 0)
        elif control_mode == "ШІ (Greedy)":
            next_dir = get_next_direction(snake, food, walls, map_width, map_height)
            if next_dir != (0, 0):
                direction = next_dir
